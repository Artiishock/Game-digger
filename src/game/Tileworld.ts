...
function drawTile(g: PIXI.Graphics, type: T, px: number, py: number, tileSeed: number) {
  ...
  g.rect(px, py+TILE-5, TILE, 5).fill({ color: shadow, alpha: 0.30 })
  g.rect(px+TILE-3, py, 3, TILE).fill({ color: 0x000000, alpha: 0.10 })
  if (!wasLavaOrMagma) {
    g.rect(px, py, TILE, 3).fill({ color: 0xffffff, alpha: 0.06 })
  }
  if (type === T.GRASS) {
    g.rect(px, py+TILE-8, TILE, 8).fill({ color: 0x5a8a30, alpha: 0.35 })
  }
  // LAVA/MAGMA эллипсы убраны — рендерятся через LavaSimulation
  if (type === T.GOLD_ORE) {
    for (let i=0;i<5;i++) {
      g.circle(px+3+rng()*(TILE-6), py+3+rng()*(TILE-6), 2.5+rng()*2).fill({ color: 0xFFD700, alpha: 0.8 })
    }
    g.moveTo(px+4,py+TILE*0.5).lineTo(px+TILE-4,py+TILE*0.4).stroke({ width: 1.5, color: 0xFFD700, alpha: 0.5 })
  }
  if (type === T.IRON) {
    for (let i=0;i<4;i++) { g.circle(px+3+rng()*(TILE-6),py+3+rng()*(TILE-6),2+rng()*2).fill({ color: 0xE8E8D0, alpha: 0.7 }) }
  }
  if (type === T.COAL) {
    for (let i=0;i<4;i++) { g.circle(px+3+rng()*(TILE-6),py+3+rng()*(TILE-6),2+rng()*3).fill({ color: 0x111111, alpha: 0.9 }) }
  }
  if (type === T.CRYSTAL) {
    const cx=px+TILE/2, cy=py+TILE/2
    g.poly([cx,py+4,cx+10,cy,cx,py+TILE-4,cx-10,cy]).fill({ color: 0x7FEFEF, alpha: 0.9 })
    g.poly([cx,py+4,cx+5,cy-4,cx,cy-2,cx-5,cy-4]).fill({ color: 0xffffff, alpha: 0.4 })
  }
  if (type === T.BRICK) {
    const off = tileSeed%2===0?TILE/2:0
    g.moveTo(px,py+TILE/2).lineTo(px+TILE,py+TILE/2)
    g.moveTo(px+off,py).lineTo(px+off,py+TILE/2)
    g.moveTo(px+off+TILE/2,py).lineTo(px+off+TILE/2,py+TILE/2)
    g.moveTo(px+((off+TILE/4)%TILE),py+TILE/2).lineTo(px+((off+TILE/4)%TILE),py+TILE)
    g.stroke({ width: 1, color: 0x000000, alpha: 0.2 })
    if (rng()<0.3) { g.rect(px,py,TILE,4).fill({ color: 0x27AE60, alpha: 0.3 }) }
  }
  if (type === T.WOOD) {
    g.circle(px+TILE/2,py+TILE/2,TILE*0.28).circle(px+TILE/2,py+TILE/2,TILE*0.14).stroke({ width: 1, color: 0x000000, alpha: 0.12 })
    g.rect(px+TILE*0.45,py,TILE*0.1,TILE).fill({ color: 0x000000, alpha: 0.07 })
  }
  if (type === T.LEAF) {
    for (let i=0;i<6;i++) { g.circle(px+3+rng()*(TILE-6),py+3+rng()*(TILE-6),2+rng()*3).fill({ color: 0x1e8449, alpha: 0.4 }) }
  }
  if (type===T.DIRT||type===T.DIRT2||type===T.DIRT3||type===T.CLAY) {
    for (let i=0;i<3;i++) {
      g.ellipse(px+3+rng()*(TILE-6),py+3+rng()*(TILE-6),2+rng()*4,1.5+rng()*2.5).fill({ color: 0x000000, alpha: 0.08+rng()*0.1 })
    }
  }
  g.rect(px,py,TILE,TILE).stroke({ width: 0.5, color: 0x000000, alpha: 0.07 })
}
...
interface Chunk {
  ...
  col: number
  row: number
  /** Маска прокопа (white=грунт, чёрное=выкопано). Создаётся всегда. */
  maskRT: PIXI.RenderTexture
  /** Лава-маска чанка (может отсутствовать). */
  lavaRT: PIXI.RenderTexture | null
  /** Маска травы (может отсутствовать). */
  grassMaskRT: PIXI.RenderTexture | null
  /** Запечённая земля чанка (может отсутствовать, если нет earth-текстур). */
  earthRT: PIXI.RenderTexture | null
  /** Плоский RT итогового вида чанка при USE_BAKED_TUNNEL_MASK. */
  displayRT: PIXI.RenderTexture | null
  /** Контейнер визуального содержимого чанка. */
  content: PIXI.Container
  /** Контейнер лавы поверх земли (может отсутствовать). */
...
export class TileWorld {
  ...
  private _redrawTunnelBgDimOverlay(): void {
    ...
    this._tunnelBgDim
      .rect(TUNNEL_BG_X, 0, TUNNEL_BG_W, TUNNEL_BG_H)
      .fill({ color: TUNNEL_FLOOR_TINT_COLOR, alpha: TUNNEL_FLOOR_TINT_ALPHA })
  }
  ...
  rebuildTunnelBgFromTextures(): void {
    ...
    g.rect(TUNNEL_BG_X, 0, TUNNEL_BG_W, TUNNEL_BG_H).fill(TUNNEL_BG_FALLBACK)
  ...
  private _scratchWorldTunnelStamp(
    ...
    for (let row = rowMin; row <= rowMax; row++) {
      for (let col = colMin; col <= colMax; col++) {
        ...
        this.brush.clear()
        this.brush.poly(poly).fill(0x000000)
        this.renderer.render({ container: this.brush, target: chunk.maskRT, clear: false })
        ...
        const grassMaskRT = chunk.grassMaskRT
        if (grassMaskRT && carveGrass) {
          ...
          this._scratchGb.poly(polyG).fill(0x000000)
        ...
        if (lineFrom) {
          ...
          this.line.clear()
          this.line.moveTo(plx, ply).lineTo(lx, ly).stroke({ width: lineW, color: 0x000000 })
          this.renderer.render({ container: this.line, target: chunk.maskRT, clear: false })
          if (stats) stats.renderOps++

          if (grassMaskRT && carveGrass) {
            this._scratchGl.clear()
            this._scratchGl.moveTo(plx, ply + GRASS_MASK_HEADROOM).lineTo(lx, ly + GRASS_MASK_HEADROOM).stroke({ width: lineW, color: 0x000000 })
  ...
  scratchAt(
    ...
    if (USE_BAKED_TUNNEL_MASK && this._flattenDirtyKeys.size > 0) {
      for (const key of this._flattenDirtyKeys) {
        ...
        if (!ch) continue
        const dRT = ch.displayRT
        const cont = ch.content
  ...
  private _rebakeBakedChunk(chunk: Chunk): void {
    ...
    const displayRT = chunk.displayRT
  ...
  clearRuntimeDigging(): void {
    ...
    for (const [key, chunk] of this.chunks) {
      const maskRT = chunk.maskRT
      const lavaRT = chunk.lavaRT
      const grassMaskRT = chunk.grassMaskRT
      if (!maskRT) continue
      this._scratchDb.clear().rect(0, 0, maskRT.width, maskRT.height).fill(0xffffff)
      this.renderer.render({ container: this._scratchDb, target: maskRT, clear: true })
      if (grassMaskRT) {
        this._scratchDb.clear().rect(0, 0, grassMaskRT.width, grassMaskRT.height).fill(0xffffff)
        this.renderer.render({ container: this._scratchDb, target: grassMaskRT, clear: true })
      }
      if (lavaRT) {
        this._scratchDb.clear().rect(0, 0, lavaRT.width, lavaRT.height).fill(0x000000)
  ...
  private _applyCavePathToExistingChunks(cave: CavePath): number {
    ...
    for (let row = rowMin; row <= rowMax; row++) {
      for (let col = colMin; col <= colMax; col++) {
        ...
        const lavaContainer = chunk.lavaContainer
  ...
  update(
    ...
    for (const [key, chunk] of this.chunks) {
      if (chunk.col < colMin-cull || chunk.col > colMax+cull ||
        ...
        const evictMaskRT = chunk.maskRT
        const evictGrassRT = chunk.grassMaskRT
        ...
        chunk.lavaRT?.destroy(true)
        chunk.earthRT?.destroy(true)
        chunk.displayRT?.destroy(true)
  ...
  private _buildChunk(col: number, row: number) {
    ...
    if (hasEarth) {
      ...
      fb.rect(-CHUNK_VISUAL_OVERLAP_PX, -topOverlap, CPW + CHUNK_VISUAL_OVERLAP_PX * 2, visualH)
        .fill(0x5a2d14)
      content.addChild(fb)
    }
    ...
    if (this.renderer) {
      ...
      this._scratchDb.clear().rect(0, 0, MASK_W, MASK_H).fill(0xffffff)
      this.renderer.render({ container: this._scratchDb, target: maskRT, clear: true })
      if (this._buildDiagnostics.active) this._buildDiagnostics.renderCalls++
      if (grassMaskRT) {
        this._scratchWb.clear().rect(0, 0, MASK_W, MASK_H + GRASS_MASK_HEADROOM).fill(0xffffff)
    ...
    this.chunks.set(key, { gfx: container, col, row, maskRT, lavaRT, grassMaskRT, earthRT, displayRT, content, lavaContainer })
    if (this._buildDiagnostics.active) {
  ...
  private _applyCavePathToChunk(
    ...
    if (cave.rect) {
      ...
      if (rx0 + rw > 0 && rx0 < CPW && ry0 + rh > 0 && ry0 < CPH) {
        hasContent = true
        g.roundRect(rx0, ry0, rw, rh, cr).fill(0x000000)
      }
      ...
      if (lx + p.r >= 0 && lx - p.r <= CPW && ly + p.r >= 0 && ly - p.r <= CPH) {
        hasContent = true
        g.circle(lx, ly, p.r).fill(0x000000)
      }
    } else {
    ...
    if (lavaRT && lavaContainer) {
      ...
      if (cave.rect) {
        ...
        gWhite.roundRect(rx0, ry0, 2 * hw, 2 * hh, cr).fill(0xffffff)
      } else if (cave.points.length === 1) {
        const p = cave.points[0]!
        gWhite.circle(p.x - offX, p.y - offY, p.r).fill(0xffffff)
      } else {
        for (let i = 0; i < cave.points.length - 1; i++) {
  ...
  destroy() {
    ...
    for(const c of this.chunks.values()){
      ...
      const dMaskRT = c.maskRT
      const dGrassRT = c.grassMaskRT
      if (dMaskRT) this._maskRTPool.push(dMaskRT)
      if (dGrassRT) this._grassMaskRTPool.push(dGrassRT)
      c.gfx.destroy({children:true})
      c.lavaRT?.destroy(true)
      c.earthRT?.destroy(true)
      c.displayRT?.destroy(true)
    }
    this.chunks.clear()
